...
def run_genetic_algorithm(
    ...
    for generation in range(1, number_of_iterations + 1):
        ...
        # 竞争：将子代个体（offspring）与当前种群（population）进行比较，选择适应度值较好的个体更新种群。
        ...
        step.append(generation)
        objective_value.append(population[0].objective_value)

    print(f'[GA] Best individual found on iteration: {best_iteration}.')
    best_individual.normalize_final_assignments()

    return best_individual.assignments, best_individual.objective_value, step, objective_value
